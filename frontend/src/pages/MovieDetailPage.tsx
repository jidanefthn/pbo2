...
export default function MovieDetailPage() {
        ...
        if (Array.isArray(allSchedules)) {
          ...
          const formattedDates = uniqueDates.map(dateStr => {
            ...
            if (diffDays === 0) label = 'Hari Ini';
              ...
              label = targetDate.toLocaleDateString('id-ID', { weekday: 'long' });
            }
  ...
  const filteredSchedules = schedules.filter(sch => (sch.showDate || sch.date) === selectedDate);
  ...
  const handleOrder = () => {
    ...
    // Mengirim data manifes tiket lengkap ke halaman pembayaran terintegrasi
  ...
  if (isLoading) {
    return <div className="loading-text" style={{ padding: '50px', textAlign: 'center', color: '#82ebd5' }}>Memuat detail film dan jadwal...</div>;
  }

  return (
    <div className="movie-layout-modern">
      <nav className="navbar-detail">
        <img src={logoImage} alt="Logo" className="nav-logo-short" onClick={() => navigate('/')} />
      </nav>
          ...
          <div className="modern-content-container">
            <div className="movie-header-row">
              <div className="poster-small-wrapper">
                <img src={movie.posterUrl || 'https://placehold.co/400x600/111827/ffffff?text=NO+POSTER'} alt={movie.title} className="poster-small"/>
              </div>
            ...
            <div style={{ backgroundColor: 'rgba(11, 15, 25, 0.6)', padding: '30px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', marginTop: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', marginBottom: '30px', overflowX: 'auto', paddingBottom: '10px' }}>
                {availableDates.length > 0 ? (
                  availableDates.map(d => (
                    ...
                    >
                      <div style={{ fontSize: '13px', marginBottom: '4px', textTransform: 'capitalize' }}>{d.label}</div>
                      <div style={{ fontWeight: 'bold', fontSize: '15px' }}>{d.date.split('-').reverse().join('/')}</div>
                    </button>
                  ))
                ) : (
                  <div style={{ color: '#9ca3af', padding: '10px 0' }}>Tidak ada tanggal tayang aktif dari admin untuk film ini.</div>
                )}
              </div>
              ...
              {Object.keys(schedulesByStudio).length > 0 ? (
                Object.keys(schedulesByStudio).map(studio => (
                  <div key={studio} style={{ Richmond: 'transparent', marginBottom: '25px' }}>
                    <h4 style={{ color: '#9ca3af', fontSize: '15px', marginBottom: '12px', fontWeight: 'normal' }}>{studio}</h4>
                    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                      {schedulesByStudio[studio].map(item => {
                        ...
                        const isSelected = selectedStudio === studio && selectedTime === cleanTime;
                        
                        return (
                          <button
                            key={item.id}
                            onClick={() => handleTimeSelect(studio, cleanTime, item.id)}
                            style={{
                              padding: '10px 24px',
                              borderRadius: '8px',
            ...
            <div className="modern-action-menu">
              <button className="btn-action-cancel" onClick={() => navigate('/')}>Batalkan</button>
